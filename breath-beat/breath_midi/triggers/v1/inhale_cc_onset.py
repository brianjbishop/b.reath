...
class InhaleCcOnsetTrigger(TriggerStrategy):
    """
    Fires a single CC message on inhale phase entry.

    One shot on phase transition — not continuous.
    Debounced using the same debounce_ms as inhale_onset.
    CC number and value are mutable at runtime via set_cc().
    """

    id = "inhale_cc_onset"
    display_name = "Inhale CC onset"
    ...
    def on_frame(self, frame: FeatureFrame, ctx: TriggerContext) -> list[TriggerEvent]:
        # Reuse inhale_onset config for enabled flag and debounce_ms.
