...
class DeviceRuntime:
    ...
    def __init__(self, config: ConfigModel, shared_sink: MidiSink, midi_channel: int | None = None) -> None:
        ...
        self._router = MidiRouter(self._config, midi=shared_sink, midi_channel=self._midi_channel)
    ...
    def set_notes(self, inhale_note: int, exhale_note: int) -> None:
        ...
        with self._lock:
            ...
            self._router = MidiRouter(new_cfg, midi=self._shared_sink, midi_channel=self._midi_channel)
    ...
    def set_midi_channel(self, channel: int) -> None:
        """Reassign the MIDI channel for all outgoing messages. Takes effect immediately."""
        with self._lock:
            self._midi_channel = channel
            self._router = MidiRouter(self._config, midi=self._shared_sink, midi_channel=channel)
