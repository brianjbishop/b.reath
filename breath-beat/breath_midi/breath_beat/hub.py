...
class BreathBeatHub:
    """
    Orchestrates the Breath Beat pipeline:
    ...
    Independent instance from EveryBreathHub — shares no state.
    """
    ...
    def start_listening(self, out_port: str | None = None) -> None:
        ...
        if self._midi_sink is None:
            ...
            try:
                ...
                print(f"[bb] MIDI open failed: {exc}")
        ...
        print("[bb] Breath Beat listening on port 8001")
    ...
    def set_midi_channel(self, uuid: str, channel: int) -> None:
        ...
        runtime = self._runtimes.get(uuid)
        if runtime is not None:
            runtime.set_midi_channel(channel)
        print(f"[bb] {uuid[:8]} → MIDI ch {channel}")
    ...
    def _on_new_device(self, uuid: str) -> None:
        ...
        if uuid not in self._runtimes:
            ...
            runtime = DeviceRuntime(device_cfg, self._midi_sink, entry.midi_channel)
            runtime.set_cons_n(0)
            self.registry.set_cons_n(uuid, 0)
            self._runtimes[uuid] = runtime
            with self._lock:
                self._waveform_bufs[uuid] = deque(maxlen=_WAVEFORM_MAXLEN)
        print(
            f"[bb] Device connected: {uuid} | "
    ...
    def _on_timeout(self, uuid: str) -> None:
        ...
        print(f"[bb] Device disconnected: {uuid}")
