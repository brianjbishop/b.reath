...
class MidiRouter:
    def __init__(self, cfg: ConfigModel, midi: MidiSink, midi_channel: int | None = None):
        self.cfg = cfg
        self.midi = midi
    ...
    def handle(self, event: TriggerEvent) -> None:
        ch = (self._midi_channel - 1) if self._midi_channel is not None else int(self.cfg.midi.channel)
