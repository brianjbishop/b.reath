...
class BreathBeatAnimation:
    """
    Self-contained breath guide animation panel for the Breath Beat tab.
    ...
    def build(self) -> None:
        ...
        dpg.add_text("", tag="bb_anim_phase_label", color=(180, 180, 180))
    ...
    def update(self, dt: float) -> None:
        ...
        if dpg.does_item_exist("bb_anim_phase_label"):
            dpg.set_value("bb_anim_phase_label", self._phase)
    ...
    def stop(self) -> None:
        ...
        if dpg.does_item_exist("bb_anim_phase_label"):
            dpg.set_value("bb_anim_phase_label", "")
        if dpg.does_item_exist("bb_anim_start_stop"):
            dpg.configure_item("bb_anim_start_stop", label="Start")

    # ── private helpers ───────────────────────────────────────────────────────

    def _update_circle(self, radius: float, stopped: bool = False) -> None:
        if not dpg.does_item_exist("bb_anim_circle"):
            return
        if stopped:
            color = _COLOR_STOPPED
            ...
            color = _COLOR_EXHALE
        dpg.configure_item("bb_anim_circle", radius=radius, fill=color, color=color)

    # ── callbacks ─────────────────────────────────────────────────────────────

    def _on_bpm_minus(self) -> None:
        self._bpm = max(20.0, self._bpm - 1.0)
        if dpg.does_item_exist("bb_anim_bpm"):
            dpg.set_value("bb_anim_bpm", int(round(self._bpm)))
    ...
    def _on_start_stop(self) -> None:
        if self._running:
            ...
            self._phase_t = 0.0
            if dpg.does_item_exist("bb_anim_start_stop"):
                dpg.configure_item("bb_anim_start_stop", label="Stop")
