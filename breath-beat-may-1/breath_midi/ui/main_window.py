...
from breath_midi.ui.breath_beat_tab import BreathBeatTab
...
def run_ui(
    ...
    eb_hub: EveryBreathHub | None = None,
) -> None:
    BreathMidiDpgUI(runtime, store, midi_bus, eb_hub).run()
...
class BreathMidiDpgUI:
    def __init__(
        ...
        self._eb_hub = eb_hub
        self._eb_tab: EveryBreathTab | None = None
        self._gb_tab: GroupBreathTab | None = None
        self._bb_hub = BreathBeatHub(runtime.config)
        self._bb_tab: BreathBeatTab | None = None
        self._tab_manager: TabActivityManager | None = None
        self._gb_tab_active: bool = False
        self._bb_tab_active: bool = False
    ...
    def run(self) -> None:
        ...
        # a circle toggle.  Reflect the stopped state in the Start/Stop buttons.
        self._sync_start_stop_buttons()
        self._tab_manager = TabActivityManager(self.runtime, self._eb_hub, self._bb_hub)
    ...
    def _show_qr(self) -> None:
        port = int(self.runtime.config.input.osc_port)
        show_qr_popup(port, "breath-choir")

    def _build_coming_soon_tab(self) -> None:
        with dpg.child_window(border=False, width=-1, height=-1):
            dpg.add_spacer(height=200)
            dpg.add_text("Coming soon", indent=450, color=(160, 160, 160))

    _TAB_CONTENT_TAGS: dict[str, str] = {
        "Single Breath": "tab_content_single",
        "Every Breath":  "tab_content_every",
        "Group Breath":  "tab_content_group",
        "Breath Beat":   "tab_content_breath_beat",
    }

    def _navigate_tab(self, tab_name: str) -> None:
        """Show the selected tab content panel; hide all others."""
        for name, tag in self._TAB_CONTENT_TAGS.items():
            if dpg.does_item_exist(tag):
                dpg.configure_item(tag, show=(name == tab_name))

    def _build(self) -> None:
        # ── Circle toggle themes (yellow = active, gray = inactive) ───────────
        with dpg.theme(tag="theme_circle_yellow"):
            with dpg.theme_component(dpg.mvButton):
                dpg.add_theme_color(dpg.mvThemeCol_Button,        (255, 200,   0, 255))
                dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered,  (255, 200,   0, 180))
                dpg.add_theme_color(dpg.mvThemeCol_ButtonActive,   (255, 200,   0, 255))
        ...
        with dpg.theme(tag="theme_circle_gray"):
            with dpg.theme_component(dpg.mvButton):
                ...
                dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered,  ( 80,  80,  80, 180))
        ...
        with dpg.window(
            ...
            # ── Custom tab navigation: label buttons + activity circles ──────────
            # Tab label clicks navigate (show/hide content) only.
            # Circle buttons are the sole way to start/stop each tab's processes.
            with dpg.group(horizontal=True, tag="tab_header_row"):
                dpg.add_button(
                    label="Single Breath",
                    tag="tab_btn_single",
                    callback=lambda: self._navigate_tab("Single Breath"),
                )
                dpg.add_spacer(width=4)
                dpg.add_button(
                    tag="circle_single", label="", width=14, height=14,
                    callback=lambda: self._tab_manager.toggle("Single Breath"),
                )
                dpg.bind_item_theme("circle_single", "theme_circle_gray")
                dpg.add_spacer(width=16)
                dpg.add_button(
                    label="Every Breath",
                    tag="tab_btn_every",
                    callback=lambda: self._navigate_tab("Every Breath"),
                )
                dpg.add_spacer(width=4)
                dpg.add_button(
                    tag="circle_every", label="", width=14, height=14,
                    callback=lambda: self._tab_manager.toggle("Every Breath"),
                )
                dpg.bind_item_theme("circle_every", "theme_circle_gray")
                dpg.add_spacer(width=16)
                dpg.add_button(
                    label="Group Breath",
                    tag="tab_btn_group",
                    callback=lambda: self._navigate_tab("Group Breath"),
                )
                dpg.add_spacer(width=4)
                dpg.add_button(
                    tag="circle_group", label="", width=14, height=14,
                    callback=lambda: self._on_group_breath_toggle(),
                )
                dpg.bind_item_theme("circle_group", "theme_circle_gray")
                dpg.add_spacer(width=16)
                dpg.add_button(
                    label="Breath Beat",
                    tag="tab_btn_breath_beat",
                    callback=lambda: self._navigate_tab("Breath Beat"),
                )
                dpg.add_spacer(width=4)
                dpg.add_button(
                    tag="circle_breath_beat", label="", width=14, height=14,
                    callback=lambda: self._on_breath_beat_toggle(),
                )
                dpg.bind_item_theme("circle_breath_beat", "theme_circle_gray")
            dpg.add_separator()

            # ── Tab content panels (show/hide on navigation) ──────────────────
            with dpg.group(tag="tab_content_single", show=True):
                with dpg.group(horizontal=True):
                    ...
                    with dpg.child_window(
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            dpg.add_spacer(width=8)
                            dpg.add_button(label="Show QR", callback=lambda: self._show_qr())
            ...
            with dpg.group(tag="tab_content_every", show=False):
                if self._eb_hub is not None:
                    self._eb_tab = EveryBreathTab(
                        hub=self._eb_hub,
                        parent_tag="tab_content_every",
                    )
                    self._eb_tab.build()
                else:
                    self._build_coming_soon_tab()
            ...
            with dpg.group(tag="tab_content_breath_beat", show=False):
                self._bb_tab = BreathBeatTab(
                    hub=self._bb_hub,
                    parent_tag="tab_content_breath_beat",
                )
                self._bb_tab.build()
    ...
    def _on_group_breath_toggle(self) -> None:
        self._tab_manager.toggle("Group Breath")
        self._gb_tab_active = not self._gb_tab_active
        if not self._gb_tab_active and self._gb_tab is not None:
            self._gb_tab.stop_animation()

    def _on_breath_beat_toggle(self) -> None:
        self._tab_manager.toggle("Breath Beat")
        self._bb_tab_active = not self._bb_tab_active
        if not self._bb_tab_active and self._bb_tab is not None:
            self._bb_tab.stop_animation()
    ...
    def tick(self) -> None:
        ...
        if self._eb_tab is not None:
            self._eb_tab.update()
        if self._gb_tab is not None:
            self._gb_tab.update()
        if self._bb_tab is not None:
            self._bb_tab.update()
