...
from breath_midi.breath_beat.hub import BreathBeatHub, DeviceUISnapshot
...
class BreathBeatBottomPanel:
    """
    Collapsible per-device strip panel for the Breath Beat tab.
    ...
    Strips are rebuilt only when the UUID list changes.  Per-frame refresh
    uses configure_item on drawlist rect fill — no delete/redraw.
    ...
    def __init__(self, hub: BreathBeatHub, parent_tag: str) -> None:
        ...
        self._panel_tag = "bb_bottom_panel"
        self._strip_container_tag = "bb_strip_container"
        self._device_theme_tags: dict[str, int] = {}

    # ── lifecycle ─────────────────────────────────────────────────────────────

    def build(self) -> None:
        """Called once from BreathBeatTab.build() after the shared plot."""
        with dpg.child_window(
            ...
            with dpg.group(horizontal=True, tag="bb_bottom_header"):
                dpg.add_button(
                    label="v Devices",
                    tag="bb_bottom_collapse_btn",
    ...
    def update(self, snapshots: list[DeviceUISnapshot]) -> None:
        """Called every frame from BreathBeatTab.update() with already-fetched snapshots."""
    ...
    def _build_strip(self, snap: DeviceUISnapshot) -> None:
        ...
        strip_tag = f"bb_strip_{snap.uuid}"
        r, g, b = snap.color
        ...
        with dpg.child_window(
            ...
            dpg.add_text(
                snap.name[:15],
                tag=f"bb_strip_name_{snap.uuid}",
                color=(r, g, b, 255),
            )
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_drawlist(
                    width=14, height=14,
                    tag=f"bb_strip_inh_{snap.uuid}",
                )
                dpg.draw_rectangle(
                    (1, 1), (13, 13),
                    fill=inh_color,
                    color=inh_color,
                    parent=f"bb_strip_inh_{snap.uuid}",
                    tag=f"bb_strip_inh_rect_{snap.uuid}",
                )
                dpg.add_spacer(width=6)
                dpg.add_text("Ex")
                dpg.add_drawlist(
                    width=14, height=14,
                    tag=f"bb_strip_exh_{snap.uuid}",
                )
                dpg.draw_rectangle(
                    (1, 1), (13, 13),
                    fill=exh_color,
                    color=exh_color,
                    parent=f"bb_strip_exh_{snap.uuid}",
                    tag=f"bb_strip_exh_rect_{snap.uuid}",
                )
                dpg.add_spacer(width=6)
                dpg.add_text("Gate")
                ...
                dpg.add_drawlist(
                    width=14, height=14,
                    tag=f"bb_strip_gate_{snap.uuid}",
                )
                dpg.draw_rectangle(
                    (1, 1), (13, 13),
                    fill=gate_color,
                    color=gate_color,
                    parent=f"bb_strip_gate_{snap.uuid}",
                    tag=f"bb_strip_gate_rect_{snap.uuid}",
                )
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(
                    label="M",
                    tag=f"bb_strip_mute_{snap.uuid}",
                    width=24,
                    height=24,
                ...
                dpg.bind_item_theme(
                    f"bb_strip_mute_{snap.uuid}",
                    device_theme if snap.muted else "theme_circle_gray",
                )
                dpg.add_spacer(width=4)
                dpg.add_button(
                    label="S",
                    tag=f"bb_strip_solo_{snap.uuid}",
                    width=24,
                    height=24,
                ...
                dpg.bind_item_theme(
                    f"bb_strip_mode_toggle_{snap.uuid}",
                    "theme_circle_yellow",
                )
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("N:")
                dpg.add_input_int(
                    tag=f"bb_strip_cons_n_{snap.uuid}",
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("Tol:")
                dpg.add_input_float(
                    tag=f"bb_strip_cons_tol_{snap.uuid}",
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("In#:")
                dpg.add_input_int(
                    tag=f"bb_strip_inh_num_{snap.uuid}",
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("Ex#:")
                dpg.add_input_int(
                    tag=f"bb_strip_exh_num_{snap.uuid}",
            ...
            with dpg.group(
                horizontal=True,
                tag=f"bb_strip_cc_val_row_{snap.uuid}",
                show=snap.cc_mode,
            ):
                dpg.add_text("Val:")
                dpg.add_input_int(
                    tag=f"bb_strip_cc_val_{snap.uuid}",
            ...
            # Row 9: MIDI channel selector
            with dpg.group(horizontal=True):
                dpg.add_text("Ch:")
                dpg.add_input_int(
                    tag=f"bb_strip_channel_{snap.uuid}",
    ...
    def _refresh_strips(self, snapshots: list[DeviceUISnapshot]) -> None:
        for snap in snapshots:
            ...
            mute_tag = f"bb_strip_mute_{uuid}"
            ...
            solo_tag = f"bb_strip_solo_{uuid}"
            ...
            mode_toggle = f"bb_strip_mode_toggle_{uuid}"
            ...
            cc_val_row = f"bb_strip_cc_val_row_{uuid}"
    ...
    def _on_mode_toggle(self, uuid: str) -> None:
        ...
        toggle_tag = f"bb_strip_mode_toggle_{uuid}"
    ...
    def _on_cons_n_change(self, uuid: str, value: int) -> None:
        self._hub.set_cons_n(uuid, int(value))
    ...
    def _on_channel_change(self, uuid: str, value: int) -> None:
        self._hub.set_midi_channel(uuid, max(1, min(16, int(value))))
    ...
    def _on_collapse_toggle(self) -> None:
        ...
        dpg.configure_item("bb_bottom_collapse_btn", label=label)
